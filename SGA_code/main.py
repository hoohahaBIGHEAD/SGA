...
def make_train_directory(config):
    ...
    if not os.path.exists(
        ...
        os.makedirs(
            os.path.join(
                config["TRAINING_CONFIG"]["TRAIN_DIR"],
                config["TRAINING_CONFIG"]["LOG_DIR"],
            )
        )
    if not os.path.exists(
        os.path.join(
            config["TRAINING_CONFIG"]["TRAIN_DIR"],
            config["TRAINING_CONFIG"]["SAMPLE_DIR"],
        )
    ):
        os.makedirs(
            os.path.join(
    ...
    if not os.path.exists(
        os.path.join(
            config["TRAINING_CONFIG"]["TRAIN_DIR"],
            config["TRAINING_CONFIG"]["RESULT_DIR"],
        )
    ):
        os.makedirs(
            os.path.join(
    ...
    if not os.path.exists(
        os.path.join(
            config["TRAINING_CONFIG"]["TRAIN_DIR"],
            config["TRAINING_CONFIG"]["MODEL_DIR"],
        )
    ):
        os.makedirs(
            os.path.join(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="config.yml", help="specifies config yaml file"
    )
