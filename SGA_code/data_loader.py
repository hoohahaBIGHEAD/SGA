...
class DataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment == "elastic":
            augmented_reference = elastic_transform(
                np.array(reference), 1000, 8, random_state=None
            )
        ...
        return (
            fid,
            self.img_transform_gt(augmented_reference),
            self.img_transform_gt(reference),
            self.img_transform_sketch(sketch),
        )
